...
\begin{equation}
	...
	The $\rho$ and thus the matrix \textbf{Q} are design parameters. These values determine which part of the tradeoff is most important in this particular design: performance or control effort. The higher $\rho$, the more the speed with which the pendulum mass goes to the desired position is valued. In other words, the higher $\rho$ the faster the measured pendulum position goes to the desired position. The tuning of the parameters and thus the design of the LQR itself is done in Section \ref{sec:implFBcontr}. 
