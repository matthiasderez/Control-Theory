...
As described in \ref{sec:linearization}, the Jacobians of the state and measurement equation can be found by taking the derivative of the nonlinear model $\dot{\xi} = \mathbf{f}(\xi,v_c)$ to the state vector and input respectively:
\begin{equation}
\begin{split}
\mathbf{F} &= \frac{\partial \mathbf{f}}{\partial \xi}\\
\mathbf{G} &= \frac{\partial \mathbf{f}}{\partial v_{c}}
\end{split}	
...
\end{bmatrix}\\
\mathbf{G} &= \begin{bmatrix}
T_s\\\frac{-T_scos(\theta)}{L}\\T_s\bigg(-sin(\theta)\frac{v_{tan}-v_ccos(\theta)}{L} +cos(\theta) \frac{v_c sin(\theta)}{L}\bigg)
\end{bmatrix}
\end{split}
\end{equation}
...
The objective of the LQR is to find the control input $v_c[k]$ that moves the system states $\xi \neq 0$ to \textbf{0}, in a way the performance criterion $\J$ is minimized. This means a tradeoff has to be made between performane $\J_p$, the speed with which the states go to zero, and the controll effort $\J_c$. The criterion is given in Equation \ref{eq:criterionLQR}. 
\begin{equation}
	\J = \J_p + \J_c =  \sum_{k=0}^{\infty}\mathbf{\xi}[k]^T\mathbf{Q}\mathbf{\xi}[k] + v_c[k]Rv_c[k]
	\label{eq:criterionLQR}
\end{equation}
The matrix \textbf{Q} and the value R are design parameters. These values determine which part of the tradeoff is most important in this particular design: performance or control effort. 
